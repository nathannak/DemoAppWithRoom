...
class Repository @Inject constructor(
    ...
    //ADDED
    suspend fun insertIntoDB(name: String, location: Location){
            repository.provideAcronymDao(context).addStore(StoreEntity(name,location))
    }

    //ADDED
    suspend fun getAllLocationsFromDB(): List<StoreEntity>{
        return repository.provideAcronymDao(context).getAllStores()
    }
